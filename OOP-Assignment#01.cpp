...
int main() {
   int sumRecursion = 0;
   int indexNo = 0;
   int sumWithRecursion(const int nums[], int size, int index = 0) {
       if (indexNo == size) {
           return 0;
       }
       return nums[indexNo] + sumWithRecursion(nums, size, indexNo + 1);
   
   sumRecursion = sumWithRecursion(nums, size);
   
   cout<<"Sum using recursion is: "<<sumRecursion<<endl;
}
